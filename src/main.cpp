...
#if defined(NDEBUG)
#error "EXTN cannot be compiled without assertions."
#endif
 ...
 */
CFeeRate minRelayTxFee = CFeeRate(0.00001 * COIN);
...
{
    /**
     * Block 1:  191250 EXTN pre-mined for swap
     Block Reward:
     Blocks 2 - 2000 - 25  EXTN
     Blocks 2001 - Infinite:  1  EXTN
     Proof of Stake Schedule - 15% to proposals for all phases
     85% distributed to stake wallet and master node
     */

    int64_t nBudgetMultiplier = COIN;
    CAmount nSubsidy = 1 * nBudgetMultiplier;
    
    if (nHeight == 1)
        nSubsidy = CAmount(191250) * COIN; //premine
    else if (nHeight < 350)
        nSubsidy = 25 * nBudgetMultiplier;
    else if (nHeight > 1680000)
        nSubsidy = 0 * nBudgetMultiplier;
