...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EXTNMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("EXTNMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("EXTNMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("extn-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running EXTNMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
