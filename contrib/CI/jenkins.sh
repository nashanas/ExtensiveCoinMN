...
copy_build_out()
{
    if [[ 0 -lt $(ls build/out/extn-* 2>/dev/null | wc -w) ]]
    then
        now=$(date +"%Y-%m-%d")
...
rm -rf ExtensiveCoin/depends/work
rm -rf `pwd`/cache/common
make -C ExtensiveCoin/depends download SOURCES_PATH=`pwd`/cache/common
...
echo ""
bin/gbuild --commit ExtensiveCoin=$commit  ExtensiveCoin/contrib/gitian-descriptors/gitian-osx.yml
copy_build_out "osx"
print_log
...
echo ""
bin/gbuild --commit ExtensiveCoin=$commit  ExtensiveCoin/contrib/gitian-descriptors/gitian-linux.yml
...
echo ""
bin/gbuild --commit ExtensiveCoin=$commit  ExtensiveCoin/contrib/gitian-descriptors/gitian-win.yml
copy_build_out "win"
print_log
